...
# MCP Client Manager -- lifespan 中创建,agent 中使用
...
def _build_model(model_name: str):
    """OpenAI-compatible deployment (supports GLM, DeepSeek, etc.)."""
...
class AssistantAgent:
    ...
    def _create_agent(self) -> Agent[Deps, str]:
        ...
        # MCP 启用且成功注册了 MCP tools 时跳过内置 WebSearch
        # 如果 MCP 初始化失败(manager 为 None),仍保留内置 WebSearch
        mcp_ok = settings.MCP_ENABLED and self._mcp_manager is not None
        if not mcp_ok:
    ...
    def _register_tools(self, agent: Agent[Deps, str]) -> None:
        ...
        # --- MCP Tools(动态注册,使用启动时缓存的工具定义)---
        mcp_ok = settings.MCP_ENABLED and self._mcp_manager is not None
        if mcp_ok:
            from app.agents.tools.mcp_tool import register_cached_mcp_tools

            try:
                registered = register_cached_mcp_tools(agent, self._mcp_manager)
                logger.info(f"MCP tools registered: {registered}")
            except Exception as e:
...
def get_agent(
    ...
    Args:
        ...
        system_prompt: Override system prompt. ``None`` falls back to DB active template
            or hardcoded default.
        mcp_manager: Optional MCP Client Manager for dynamic tool registration.

    Returns:
        Configured AssistantAgent instance.
    """
    # 优先使用参数传入的 mcp_manager,其次使用模块全局变量
    manager = mcp_manager or _mcp_manager
...
def set_mcp_manager(manager: Any | None) -> None:
    """设置全局 MCP Client Manager(由 lifespan 调用)."""
    global _mcp_manager
