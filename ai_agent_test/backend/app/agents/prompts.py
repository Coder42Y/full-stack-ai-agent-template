...
DB prompt loading is handled by PromptTemplateService.get_active_prompt_text()
in agent_session.py. This file provides the hardcoded fallback.
"""
...
def get_system_prompt_with_rag() -> str:
    """Get the hardcoded default prompt plus RAG usage guidance.

    This is the fallback when DB prompt loading is not available.
    For DB-driven prompt loading, use PromptTemplateService.get_active_prompt_text()
    in the agent session layer.

    Returns:
        System prompt with RAG guidance appended.
    """
    return f"{DEFAULT_SYSTEM_PROMPT}\n{_RAG_GUIDANCE}"


def _rag_guidance() -> str:
    """RAG 使用指南(追加到 DB 模板后面).

    Called by agent_session.py when appending RAG guidance to a DB-loaded prompt.
    """
    return _RAG_GUIDANCE
