...
async def lifespan(app: FastAPI) -> AsyncGenerator[LifespanState, None]:
    ...
    if settings.MCP_ENABLED:
        try:
            from app.agents.mcp_client import MCPClientManager
            from app.agents.tools.mcp_tool import discover_mcp_tools

            # 注入环境变量到 MCP Server 配置
            ...
            # 发现工具并缓存(Phase 1 -- async,无 event loop 问题)
            discovered = await discover_mcp_tools(mcp_manager)
            logger.info(f"MCP tools discovered: {discovered}")

            # 只有实际发现了 MCP tools 才设置全局 manager
            ...
            if discovered:
                ...
                logger.warning(
                    "MCP enabled but no tools discovered. "
                    "Disconnecting MCP servers, falling back to built-in WebSearch."
                )
                await mcp_manager.disconnect_all()
        except Exception as e:
            logger.error(f"MCP initialization failed: {e}. MCP tools will not be available.")
