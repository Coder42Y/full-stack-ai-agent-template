...
class AgentInvocationService:
    ...
    async def _call_pydantic_ai(
        ...
        **kwargs: Any,
        ...
        # --- Resolve system prompt (priority: override > DB > hardcoded) ---
        ...
        if system_prompt is None:
            try:
                ...
                async with get_db_context() as _db:
                    _svc = PromptTemplateService(_db)
                    db_prompt = await _svc.get_active_prompt_text(category="system")
                    if db_prompt:
                        from app.agents.prompts import _rag_guidance
                        system_prompt = db_prompt + "\n\n" + _rag_guidance()
            except Exception:
                pass  # fallback to hardcoded default

        assistant = get_agent(model_name=model_name, system_prompt=system_prompt)
