    ...
    async (rating: RatingValue, commentText: string | null) => {
      ...
      try {
        ...
        if (!response.ok) {
          ...
          throw new Error(error.message || t("ratingFailed"));
        }
    ...
    async (rating: RatingValue) => {
      ...
      if (currentRating === rating) {
        ...
        try {
          ...
          if (!response.ok) {
            const error = await response.json().catch(() => ({ message: t("unknownError") }));
            throw new Error(error.message || t("ratingRemoveFailed"));
          }
          ...
          toast.error(error instanceof Error ? error.message : t("ratingRemoveFailed"));
        } finally {
          setIsLoading(false);
        }
      } else {
