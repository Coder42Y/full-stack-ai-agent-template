...
created: 2026-06-12
status: implemented
scope: M9 前端需求工作台与产品/开发/测试 MVP
---

# Spec：M9 前端需求工作台与产品/开发/测试 MVP

## 1. 背景

现有后端已经打通需求知识库 MVP API，但前端仍以通用 RAG/Knowledge Base 页面为主，浏览器演示看不到 PRD 主线。MVP 阶段先取消生产态登录鉴权复杂度，在工作台内提供“产品/开发/测试”三个业务身份选择，用于演示真实角色边界。

## 2. 产品目标

- 将 `/kb` 从通用知识库列表重构为“需求项目”入口。
- 将 `/kb/[id]` 从通用文档上传页重构为“需求工作台”。
- 前端提供产品/开发/测试身份切换，不做生产态登录鉴权分流。
...
MVP 前端不做生产态登录鉴权拆分，但展示产品/开发/测试三个业务身份。

- 默认身份为产品，可创建需求项目、上传文档、录入一句话需求、回答澄清并应用版本变更。
- 切换为开发或测试后，录入和上传入口禁用，查询/拆解可用。
- 开发和测试在变更页提交的是修改建议，后端不会直接生成新版本。
- 测试身份下拆解和回答更强调验收场景、边界值、异常路径和回归风险。
...
9. 工作台支持版本历史、最近两版结构化 diff、实时事件流、toast 和当前事件 feed 已读状态。已实现。
10. 前端导航将 KB 命名为“需求项目”或“需求”，移动端同步更新。已实现。
11. M9 相关用户可见文案默认中文，浏览器演示不出现英文主界面。已实现。
12. 生成项目的 frontend type-check 或 build 至少一项通过。已通过 `npm run type-check`。
13. 工作台支持产品/开发/测试身份切换，并将身份透传给 BFF/FastAPI。已实现。
14. 开发/测试身份下创建/上传入口禁用，变更提交为建议而不是新版本。已实现。
15. dashboard 遵循 [M10 前端界面规范与响应式验收](m10-frontend-ui-guidelines.md)，角色信息不再放入固定 KPI 小格。已实现。

## 9. 后续迭代

- 恢复生产态真实登录用户、组织成员角色和 tester 身份。
- 澄清状态和多轮回答已持久化；后续增强为产品确认后再入库的完整对话状态机。
- 草稿评论流已实现为扁平列表；后续增强评论线程/解决状态。
- 通知中心和已读状态持久化已实现；后续增强组织成员级过滤和跨 KB 汇总。
