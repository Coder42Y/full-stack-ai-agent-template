...
export function SlashCommandsManager() {
  ...
  const handleSubmit = async () => {
    ...
    if (!NAME_PATTERN.test(name)) {
      toast.error("指令名只能包含小写字母、数字和连字符，最多 32 个字符。");
      return;
    }
    if (!prompt) {
      toast.error("提示词不能为空。");
  ...
  const handleToggleCustom = async (record: UserSlashCommandRecord, next: boolean) => {
    try {
      ...
      toast.error(e instanceof Error ? e.message : "切换指令状态失败");
    }
  };
  ...
  const handleDelete = async (record: UserSlashCommandRecord) => {
    ...
    try {
      ...
      toast.success(`/${record.name} 已删除。`);
    } catch (e) {
      toast.error(e instanceof Error ? e.message : "删除指令失败");
    }
  };
    ...
    <div className="space-y-8">
      ...
      <section className="space-y-3">
        <div className="flex items-baseline justify-between gap-3">
          <div>
            <h3 className="text-foreground text-sm font-semibold">内置指令</h3>
            <p className="text-foreground/55 mt-0.5 text-xs">
              不需要的指令可以关闭，关闭后不会出现在对话指令面板中。
            </p>
          </div>
        </div>
      ...
      <section className="space-y-3">
        <div className="flex items-baseline justify-between gap-3">
          <div>
            <h3 className="text-foreground text-sm font-semibold">自定义指令</h3>
            <p className="text-foreground/55 mt-0.5 text-xs">
              保存常用需求提示词。在对话中输入 <code>/name</code> 即可发送对应提示。
            </p>
          </div>
      ...
      <Dialog open={editingId !== null} onOpenChange={(o) => !o && closeDialog()}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>
              {editingId === "new" ? "新建自定义指令" : `编辑 /${draftName}`}
            </DialogTitle>
          </DialogHeader>
          <div className="space-y-4">
            ...
            <div>
              ...
              <Textarea
                ...
                placeholder="把当前需求对话总结为可执行任务清单。"
                rows={6}
                maxLength={10_000}
                className="mt-1.5 font-mono text-sm"
              />
              <p className="text-foreground/45 mt-1 text-[11px]">
                输入 <code>/{draftName || "name"}</code> 时会作为普通用户消息发送。
              </p>
            </div>
            {editingId !== "new" && (
